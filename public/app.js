...
if (isAdmin) {
    ...
    async function loadVoteCounts() {
        const res = await fetch(`/api/admin/counts?t=${Date.now()}`);
        const counts = await res.json();
        const html = counts.map(c => `<strong>${c.name}</strong>: ${c.votes}`).join(' &nbsp; | &nbsp; ');
        const container = document.getElementById('vote-counts');
        if(container) container.innerHTML = html || 'No votes yet';
    }
